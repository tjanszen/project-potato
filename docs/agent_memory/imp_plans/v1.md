...
## PHASE 1X: Minimal Frontend for Auth Testing
...
- **Risks/Assumptions:**
  - Backend CORS supports dev client origin
  - Form is unstyled and dev-only
- **Exit Criteria (objective, testable):**
  - Form submits to backend and creates user
...
## PHASE 1C: User Profile & Integration
...
- **Exit Criteria (objective, testable):**
  - User can complete full flow: register → login → mark days → see visual feedback
