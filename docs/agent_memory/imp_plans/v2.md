...
CREATE TABLE runs (
  ...
  CHECK (day_count = upper(date_range) - lower(date_range) + 1),
  CHECK (upper(date_range) >= lower(date_range))
);
...
-- Performance indexes
CREATE INDEX idx_runs_user_enddate ON runs(user_id, upper(date_range) DESC);
...
ALTER TABLE day_marks ADD CONSTRAINT unique_user_date UNIQUE(user_id, date);
```
...
DELETE FROM day_marks WHERE user_id = ? AND date = ?;
...
INSERT INTO day_marks (user_id, date, value) VALUES (?, ?, true);
...
**Timezone Changes:**
- Use timezone at time of day marking
- DST transitions: 23-hour and 25-hour days remain consecutive
- User timezone migration: re-calculate runs with new timezone
...
AND EXISTS (SELECT 1 FROM day_marks WHERE user_id = run_totals.user_id AND created_at > run_totals.updated_at);
```
...
GET /api/v2/runs/active:
  ...
  response: 
    run: {"id": "uuid", "start_date": "2025-06-10", "end_date": "2025-06-15", "day_count": 6, "active": true}
    # null if no active run
...
**Goal:** Create runs table schema with non-overlap guarantees, single active run enforcement, and optimized indexes. Migration planning only - no code implementation yet.

### Tasks
- Design runs table schema incorporating Phase 0 research findings
- Implement PostgreSQL EXCLUDE constraints for non-overlapping date ranges
- Create SQLite trigger-based overlap prevention as fallback
- Add database indexes for efficient run queries (user_id + date range)
- Design foreign key relationships (runs → users, maintain day_marks integrity)
- Update shared/schema.ts with runs model and Drizzle relations
- Create migration plan with rollback procedures
- Establish invariant validation queries for health checks

### Risks/Assumptions
- PostgreSQL daterange + EXCLUDE USING gist available in deployment environment
- Database migration executes successfully without corrupting existing day_marks
- Index performance meets <10ms query time targets for run lookups
- Foreign key constraints don't impact existing day marking performance

### Exit Criteria (Objective, Testable)
- Runs table created with all invariant constraints enforced
- Non-overlapping date range constraint active (PostgreSQL) or trigger-based (SQLite)  
- Unique constraint on active runs per user verified
- Foreign key relationships established without breaking existing functionality
- Database indexes demonstrate <10ms query performance on test data
- Migration script tested on copy of production data without errors
...
VALUES ('test-user', '2025-06-01', '2025-06-03', 3, true);
INSERT INTO runs (user_id, start_date, end_date, day_count, active)
VALUES ('test-user', '2025-06-02', '2025-06-04', 3, true); 
-- Should fail with overlap violation

-- Verify index performance
EXPLAIN ANALYZE SELECT * FROM runs WHERE user_id = 'test-user' ORDER BY end_date DESC;
-- Should show index usage and <10ms execution time
```
...
**Rollback Plan:** Drop runs table, remove foreign key constraints, revert shared/schema.ts  
**Feature Flag(s):** ff.potato.runs_v2 + default OFF + schema gated

---

## PHASE 6B: Algorithm & Transactions

**Goal:** Finalize transaction boundaries and per-user serialization patterns. Refine idempotent extend/merge pseudocode from Phase 0 research into production-ready implementation guide.

### Tasks
- Map invariants to specific transaction steps and isolation levels
- Design per-user serialization strategy (row-level locking vs application locks)
- Implement idempotent run extend/merge/split operations  
- Define transaction boundary scoping (narrow locks on affected runs only)
- Document race condition scenarios and mitigation strategies
- Create transaction retry logic for concurrent day marking attempts
- Design deadlock prevention through consistent lock ordering
- Establish performance targets: <10ms transaction duration, <5 rows locked per operation

### Risks/Assumptions
- Row-level locking (FOR UPDATE) performs adequately under concurrent load
- PostgreSQL transaction isolation level (READ COMMITTED) sufficient for consistency
- Deadlock scenarios rare with proper lock ordering (user_id, then start_date)
- Transaction retry logic doesn't create infinite loops under pathological load
...
- Idempotent operations handle duplicate requests without side effects
- Race conditions documented with specific mitigation strategies  
...
-- Terminal 2: 
INSERT INTO day_marks (user_id, date) VALUES ('test-user', '2025-06-15');
-- Should block until Terminal 1 commits

-- Test idempotent operations
SELECT perform_run_calculation('test-user', '2025-06-15');
SELECT perform_run_calculation('test-user', '2025-06-15'); -- Second call should be no-op
...
-- Should return 0 rows for cutover approval
