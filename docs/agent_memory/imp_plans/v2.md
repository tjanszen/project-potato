...
## Overview
Phase-gated development plan for adding runs and totals tracking to the habit tracking web app. Users can now see their total "No Drink" days and track consecutive day runs. All features ship behind feature flag `ff.potato.runs_v2` (default OFF).
...
- **Tasks:**
  - Create runs table with user_id, start_date, end_date, day_count, active columns
...
- **Evidence to Collect:**
  ...
  # Verify active column exists: user_id, start_date, end_date, day_count, active
...
- **Tasks:**
  ...
  - Implement longest run highlighting
...
- **Exit Criteria (objective, testable):**
  - API endpoints respond within <500ms response time
...
- **Performance:** API endpoints respond within <500ms response time
