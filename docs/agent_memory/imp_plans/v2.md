...
## 🛠️ Patch Plan: Schema Fix & Validation (v2)
...
- Compare against Drizzle's `shared/schema.ts`

👉 **Output:** Confirm mismatch (table has `date`, Drizzle expects `local_date`).
...
👉 **Output:** Schema updated, `local_date` column present.
...
👉 **Output:** New row for marked day appears with `local_date=2025-09-08`.
...
### Blast Radius
- **Low** — affects only one column name in one table.  
- **Safe** — preserves existing rows.  
- **Scoped** — does not touch unrelated tables or schema.
